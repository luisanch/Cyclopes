...
for(k=capture_params.first+1:capture_params.last)
    ...
    residue_difference = sum(abs(double(ReferenceImage.I(WarpedImage.index)) - WarpedImage.I(WarpedImage.index)));
    disp(residue_difference)
    if(abs(residue_difference) > tracking_param.threshold)
    disp('hey')
    change_counter = 1;
    end
...
tracking_params.change_every = 200;
tracking_params.threshold = 205000;
% Saving Results
